...
export const MovieCard = ({ movie, ratingStats, platforms, priority }: MovieCardProps) => {
  const { title, description, youtubeId, dailymotionId, okId, vimeoId, externalUrl, year, thumbnailUrl } = movie
    ...
    <article className="bg-card rounded-card overflow-hidden flex flex-col h-full group/card transition-all duration-300 hover:gold-glow hover:-translate-y-1">
      ...
      {!copyrightMode && (
        <div className="flex-shrink-0">
          <YoutubeEmbed youtubeId={youtubeId} dailymotionId={dailymotionId} okId={okId} vimeoId={vimeoId} title={title} thumbnailUrl={thumbnailUrl} priority={priority} />
        </div>
      )}
      ...
      <div className="p-4 flex-1 flex flex-col gap-2">
        ...
        {!youtubeId && !dailymotionId && !okId && !vimeoId && !externalUrl ? (
