...
function getSource(movie: Movie) {
  if (movie.youtubeId) {
    return { url: `${getYouTubeEmbedUrl(movie.youtubeId)}&autoplay=1`, label: 'YouTube' }
  }
  if (movie.dailymotionId) {
    return { url: getDailymotionEmbedUrl(movie.dailymotionId), label: 'Dailymotion' }
  }
  if (movie.okId) {
    return { url: getOkEmbedUrl(movie.okId), label: 'OK.ru' }
  }
  if (movie.vimeoId) {
    return { url: getVimeoEmbedUrl(movie.vimeoId), label: 'Vimeo' }
  }
  return null
}
